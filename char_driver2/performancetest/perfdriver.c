...
MODULE_DESCRIPTION("Performance tester");

//major 64 for test //number of devices are 0.
#define SW2_MAJOR 64
#define SW2_MINOR 0
...
static int devno;
static int err;
static uint8_t gpio = 19; //gpio of SW2.
static int irq_line;

//interrup handler
static irqreturn_t gpio_isr_handler(int irq_line, void* dev_id){

  uint8_t tmp = (gpio_get_value(gpio) > 0 ? 0 : 1);
  gpio_set_value(gpio, tmp);
...
static int sw2_init(void){
    //request GPIO of SW2
    printk("Init performance tester\n");
    err = gpio_request(gpio, "perf");
    if(err < 0){
      goto err_req;
    }
    //Set gpio directions
    ...
    err = register_chrdev_region(devno, NUM_OF_DEVS, "perf");
    if(err < 0){
      goto err_dir;
    }
    //Char device init
    ...
    //get irq_line
    irq_line = gpio_to_irq(13);
    printk("irqline of 13 is %d\n", irq_line);

    //request and enable irq
    err = request_irq(irq_line, gpio_isr_handler, IRQF_TRIGGER_FALLING, "perf", &sw_dev);
    if(err < 0){
      if(err == -EBUSY){
